...
class ModelMAE(nn.Module):
    def __init__(
        ...
        self.perception_lane_mask_token = nn.Parameter(torch.Tensor(1, 1, embed_dim))
        ...
        self.lane_pred = nn.Linear(embed_dim, 30 * 2)
        self.perception_lane_pred = nn.Linear(embed_dim, 30 * 2)
    ...
    def initialize_weights(self):
        ...
        nn.init.normal_(self.perception_lane_mask_token, std=0.02)
    ...
    def agent_random_masking(
        hist_tokens, mask_ratio, future_padding_mask, num_actors
    ):
    ...
    def lane_random_masking(x, future_mask_ratio, key_padding_mask):
        ...
        # print("num_tokens")
        # print(num_tokens)
        len_keeps = torch.floor(num_tokens * (1 - future_mask_ratio)).int()
        # print(len_keeps)
    ...
    def forward(self, data):
        hist_padding_mask = data["x_padding_mask"][:, :, :30]
        hist_feat = torch.cat(
            [
                data["x"],
        ...
        future_padding_mask = data["x_padding_mask"][:, :, 30:]
        ...
        perception_lane_feat += self.lane_type_embed
        ...
        # print("hist_masked_tokens")
        # print(data["lane_key_padding_mask"])
        ...
        x = torch.cat(
            # [hist_masked_tokens, fut_masked_tokens, lane_masked_tokens, perception_lane_masked_tokens], dim=1
            [hist_masked_tokens, lane_masked_tokens, perception_lane_masked_tokens], dim=1
        )
        key_padding_mask = torch.cat(
            # [hist_key_padding_mask, fut_key_padding_mask, lane_key_padding_mask, perception_lane_key_padding_mask],
            [hist_key_padding_mask, lane_key_padding_mask, perception_lane_key_padding_mask],
            dim=1,
        )
        ...
        Nh, Nl, Np = (
            ...
            perception_lane_masked_tokens.shape[1],
        )
        ...
        x_decoder = torch.cat(
            # [decoder_hist_token, decoder_fut_token, decoder_lane_token, decoder_perception_lane_token], dim=1
            [decoder_hist_token, decoder_lane_token, decoder_perception_lane_token], dim=1
        )
        
        # print("size")
        # print(x_decoder.size())
        # print(self.decoder_pos_embed(pos_feat).size())          
        ...
        perception_lane_pred_loss = F.mse_loss(
            perception_lane_pred[perception_lane_reg_mask], perception_lane_normalized[perception_lane_reg_mask]
        )
        
        # print(hist_token.size())
        # hist pred loss
        x_hat = self.history_pred(hist_token).view(-1, 30, 2)
        x = (data["x_positions"] - data["x_centers"].unsqueeze(-2)).view(-1, 30, 2)
        ...
        loss = (
            ...
            + self.loss_weight[2] * perception_lane_pred_loss
        )
        ...
        out = {
            ...
            "perception_pred_loss": perception_lane_pred_loss.item(),
        }
